...
namespace AerovelenceMod.NPCs.Bosses.SkyTest
{
    [AutoloadBossHead]
    public class TestingHead : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("Testing Worm"); //DONT Change me
            Main.npcFrameCount[npc.type] = 1;
        }
        public override void SetDefaults()
        {
            npc.lifeMax = 120000;        //this is the npc health
            npc.damage = 100;    //this is the npc damage
            npc.defense = 15;         //this is the npc defense
            npc.knockBackResist = 0f;
            npc.width = 54; //this is where you put the npc sprite width.     important
            npc.height = 72; //this is where you put the npc sprite height.   important
            npc.boss = true;
	...
	public override void NPCLoot()
        ...
        {
            npc.chaseable = !NPC.AnyNPCs(mod.NPCType("WormProbeCirclerMeme"));
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && !segmentsCreated)
            {
                ...
                {
                    int segment = mod.NPCType("TestingBody");
                    if (i == numSegments)
                    {
                        segment = mod.NPCType("TestingTail");
                    }
                    ...
                    for (int i = 0; i < segmentIDs.Count; i++)
                    {
                        if (i % 3 == 0)
                        {
                            for (int j = 0; j < 4; j++)
                            {
                                ...
                                int spawned = NPC.NewNPC((int)(npc.Center.X), (int)(npc.Center.Y), mod.NPCType("WormProbeCirclerMeme"), npc.whoAmI, segmentIDs[i], angle);
                            }
                        }
                    }
                    HPIncrements[0] = true;
