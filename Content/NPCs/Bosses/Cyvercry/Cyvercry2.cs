...
namespace AerovelenceMod.Content.NPCs.Bosses.Cyvercry 
{
    ...
    public class Cyvercry2 : ModNPC
    {
        ...
        public override void ModifyHitByItem(Player player, Item item, ref int damage, ref float knockback, ref bool crit)
        {
            if (item.DamageType == DamageClass.Melee && (NPC.velocity.Length() > 17 || whatAttack == 3))
            {
                if ((NPC.Center - Main.player[NPC.target].Center).Length() < 300)
                    damage = (int)(damage * (whatAttack == 3 && !spammingLaser ? 2 : 5));
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (DrawDeathOrb)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Main.spriteBatch.Draw(starTex2, starPos - Main.screenPosition, starTex2.Frame(1, 1, 0, 0), Color.HotPink, NPC.rotation + MathHelper.ToRadians(timer * 0.7f), starTex2.Size() / 2, deathOrbScale, SpriteEffects.None, 0);
            ...
            //GigaBeam
            if (whatAttack == 5 && timer < 95)
            {
                Vector2 dirToTarget = NPC.rotation.ToRotationVector2();
                Utils.DrawLine(spriteBatch, from, NPC.Center + dirToTarget * -2400, Color.HotPink * 0.2f, Color.HotPink * 0.2f, 2);
        ...
        {
            float bonusShots = isMaster ? 3 : isExpert ? 0 : -2;
                ...
                if (timer > 250  && timer < 300)
                {
                    ...
                    if (timer == 251)
                    {
                        ...
                        for (int i = 0; i < 1; i++)
                        {
                            ...
                            if (Main.projectile[a].ModProjectile is HollowPulse pulse)
                            {
                                pulse.color = Color.SkyBlue * 0.9f;
                                pulse.oval = false;
                                pulse.size = 1f + (i * 0.05f);
                            }
                        }
