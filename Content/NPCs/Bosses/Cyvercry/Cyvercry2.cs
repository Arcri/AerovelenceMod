...
namespace AerovelenceMod.Content.NPCs.Bosses.Cyvercry 
{
    ...
    public class Cyvercry2 : ModNPC
    {
...

        {
            ...
            if (drawAzzyLaser == 1 && State != AIState.DeathAnimation) //5 shot
            {
            ...
            else if (drawAzzyLaser == 2 && State != AIState.DeathAnimation) //4 shot
            {
                Player target = Main.player[NPC.target];
            ...
            //TRACK DASH DRAWING
            if (State == AIState.Clones && timer < 60)
            {
                Player target = Main.player[NPC.target];
            ...
            //GigaBeam
            if ((State == AIState.GigaBeam && timer < 95) || (State == AIState.GigaLaserSpam && timer < 50 && timer > 7))
            {
                float intensity = State == AIState.GigaLaserSpam ? 0.1f : 0.2f;
            ...
            if (!fadeDashing)
            {
                Color col = Color.White * ((NPC.velocity.Length() > 18f || State == AIState.GigaLaserSpam || State == AIState.SplitLaser) ? 0.5f : 0.25f); // 0.25f | 0.1f
        ...
        public override void FindFrame(int frameHeight)
        {
            ...
            if (NPC.frameCounter >= 10)
            {
                ...
                {
                    if ((!fadeDashing || !NPC.hide) && !NPC.hide && State != AIState.BallDash)
                    {
                        for (int i = 0; i < 0; i++) //5
                        {
        ...
        public override bool? DrawHealthBar(byte hbPosition, ref float scale, ref Vector2 position)
        {
            ...
            if (State == AIState.PinkCloneP3 && timer < 120)
                return false;
        ...
        public override void AI()
        {
            ...
            {
                firstFrame = false;
                SwitchState(AIState.IntroAnimation);
            ...
            switch (State)
            {
                ...
                case AIState.DeathAnimation:
                    ...
                    break;
                case AIState.IntroAnimation:
                    IntroAnimation(myPlayer);
                    break;
                case AIState.PhaseTransition:
                    PhaseTransition(myPlayer);
                    break;
                case AIState.IdleLaser:
                    IdleLaser(myPlayer);
                    break;
                case AIState.IdleDash:
                    IdleDash(myPlayer);
                    break;
                case AIState.AzzyLaser:
                    AzzyLaser(myPlayer);
                    break;
                case AIState.GigaBeam:
                    GigaBeam(myPlayer);
                    break;
                case AIState.Clones:
                    Clones(myPlayer);
                    break;
                case AIState.ChaseDash:
                    ChaseDash(myPlayer);
                    break;
                case AIState.WrapDash:
                    WrapDash(myPlayer);
                    break;
                case AIState.GigaLaserSpam:
                    GigaLaserSpam(myPlayer);
                    break;
                case AIState.SplitLaser:
                    SplitLaser(myPlayer);
                    break;
                case AIState.SpinPhase3:
                    SpinPhase3(myPlayer);
                    break;
                case AIState.CurvedDash:
                    CurvedDash(myPlayer);
                    break;
                case AIState.PinkCloneP3:
                    PinkCloneP3(myPlayer);
                    break;
                case AIState.Bots:
                    Bots(myPlayer);
                    break;
                case AIState.BallDash:
                    BallDash(myPlayer);
                    break;
                case AIState.EyeSword:
                    EyeSword(myPlayer);
                    break;
            }
        ...
        public void IdleLaser(Player myPlayer)
        {
            ...
            Vector2 goalPoint = new Vector2(525, 0).RotatedBy(MathHelper.ToRadians(advancer * 0.2f + startingAngBonus)); //advancer * 0.4
        ...
        public void AzzyLaser(Player myPlayer)
        {
            ...
            float shotDelay = MathHelper.Clamp(currentShot, 0, Phase3 ? 11f : 13f) * 5f; //6f

            if (currentShot == 0)
            {
                goalLocation = new Vector2(600, 0).RotatedByRandom(6.28f);
                currentShot++;
            }
        ...
        public void GigaBeam(Player myPlayer)
        {
            ...
            if (timer == 130)
            {
                ...
                if (isExpert)
                    SwitchState(AIState.GigaLaserSpam);
                else
                    SetNextAttack("Dash");
            }
        ...
        public void SplitLaser(Player myPlayer)
        {
            float reAdjustAmount = isExpert ? 0.35f : 0.125f;
            ...
            else if (advancer == 1)
            {
                if (timer <= 20)
                {
                    Vector2 vecToGoal = (splitLaserVectoGoal * 1.05f).RotatedBy(splitLaserRot);
        ...
        public void SpinPhase3(Player myPlayer)
        {
            ...
            float toPlayerVelMult = isExpert ? 0.9f : 0.7f;
            int shotModulo = isMaster ? 5 : 6;

            float turnMult = isMaster ? 0.8f : (isExpert ? 0.6f : 0.5f);
            ...
            else if (timer > 150 + extraStartupTime)
            {
                ...
                if (timer > 300 + extraStartupTime && timer < 315 + extraStartupTime)
                    newRotation = Utils.AngleTowards(NPC.rotation, (myPlayer.Center - NPC.Center).ToRotation() + MathHelper.Pi + (spinningClockwise ? 1.5f : -1.5f), 0.2f);
                else
                    newRotation = Utils.AngleTowards(NPC.rotation, (myPlayer.Center - NPC.Center).ToRotation() + MathHelper.Pi, 0.04f * turnMult); //25

                //Prevents the player from staying on top of Cyver the whole time
                if (timer > 300 + extraStartupTime && timer < 315 + extraStartupTime)
                    NPC.damage = ContactDamage / 2;
                ...
                if (timer == 400 + extraStartupTime)
                {
                    ...
                    if (advancer == 3)
                    {
                        ...
                        hasDoneMusicSync = true;
                        firstSpin = false;
                        SwitchState(AIState.SplitLaser);
                        timer = -1;
                        advancer = 0;
                        spammingLaser = false;
                    }
        ...
        public void EyeSword(Player myPlayer)
        {
            ...
            else if (advancer == 3)
            {
                ...
                if (swordEasingProgress >= 0.5f && !hasDoneSwordShot && isExpert)
                {
                    for (int j = -4; j < 5; j++)
                    {
        ...
        public void IdleDash(Player myPlayer)
        {
            ...
            float dashAccelValue = isExpert ? 70 : 50; //80
            ...
            if (timer < 105)
            {
                ...
                #region daggers
                if (timer == 80 && Phase2 && isExpert)
                {
                    int daggerCount = 3 + (isMaster ? 1 : 0);
        ...
        public void ChaseDash(Player myPlayer)
        {
            ...
            if (advancer == 20 + minusTime) //25
            {
                ...
                if (!isExpert)
                    SwitchState(AIState.EyeSword);
                else
                {
                    SwitchState(AIState.BallDash);
        ...
        public void WrapDash(Player myPlayer)
        {
            ...
            if (timer == 140)
            {
                ...
                if (advancer == repsVal)
                {
                    ...
                    if (Phase3)
                        SwitchState(AIState.CurvedDash);
                    else
                        SetNextAttack("Summon");
                }
                advancer++;
        ...
        public void BallDash(Player myPlayer)
        {
            ...
            int dash1Time = 40;
            int dash2Time = isMaster ? 55 : 60; //:55
            ...
            NPC.damage = 0;
            if (timer < 80 && timer != 39) // < 75
                NPC.dontTakeDamage = false;
            else if (timer >= 75 && timer < 100) //100
            ...
            if (timer >= dash1Time)
            {
                ...
                if (timer == dash2Time)
                {
                    ...
                    if (p.ModProjectile is LaserExplosionBall ball)
                    {
                        ...
                        ball.vel = isExpert ? 5f : 5f;
                        ball.CyverIndex = NPC.whoAmI;
                    }
                    storedVec2 = NPC.rotation.ToRotationVector2() * 25;
                }

                if (timer == dash2Time + 7)
                {
                    hideRegreGlow = true;
                    NPC.dontTakeDamage = true;
        ...
        public void CurvedDash(Player myPlayer)
        {
            ...
            float spreadShotSpread = isExpert ? 0.10f : 0.05f;
        ...
        public void Clones(Player myPlayer)
        {
            ...
            {
                timer = -1;
                SwitchState(AIState.PinkCloneP3);
                advancer = 0;
        ...
        public void PinkCloneP3(Player myPlayer)
        {
            ...
            if (timer >= 115 + startingDelay && timer < 175 + startingDelay)
            {
                ...
                if (timer < 137 + startingDelay) //132
                    pinkCloneDashValue *= (1.15f * cyverDashMult);
        ...
        public void Bots(Player myPlayer)
        {
            ...
            if (timer == startTime)
            {
                ...
                for (int i = 0; i < numberOfBots; i++)
                {
                    ...
                    if (thisBot.ModNPC is CyverBot bot)
                    {
                        ...
                        int version = (botsReps == totalReps) ? (int)(CyverBot.Behavior.PrimeLaserLong) : (int)(CyverBot.Behavior.PrimeLaser);

                        bot.State = version;
                        bot.rotIntensity = rotationSpeed;
                        bot.setGoalLocation(spawnPos);

                        if (i == 0 && (botsReps == totalReps)) //Makes the ball occur so only do so on the last wave
                            bot.Leader = true;
            ...
            //Reset normal
            if (timer == startTime + timeBetweenWaves && botsReps != totalReps)
            {
                timer = startTime - 1;
                botsReps++;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (!dead && NPC.life < 1)
            {
                ...
                dead = true;
                NPC.dontTakeDamage = true;
                SwitchState(AIState.DeathAnimation);
                timer = -1;
                advancer = 0;
                NPC.hide = false;
                fadeDashing = false;
        ...
        public void SetNextAttack(String nextAttackName)
        {
            ...
            if (!Phase3)
            {
                ...
                else if (nextAttackName == "Laser")
                {
                    ...
                    {
                        SwitchState(AIState.SpinPhase3);
                        trueSpinFalseAzzy = !trueSpinFalseAzzy;
                    }
                    else
                    {
                        SwitchState(AIState.AzzyLaser);
                        trueSpinFalseAzzy = !trueSpinFalseAzzy;
                    }
                }
                else if (nextAttackName == "Dash")
                {
                    SwitchState(AIState.WrapDash);
                }
                else if (nextAttackName == "Summon")
                {
                    ...
                    {
                        SwitchState(AIState.Clones);
                        trueCloneFalseBot = !trueCloneFalseBot;
                    }
                    else
                    {
                        SwitchState(AIState.Bots);
                        trueCloneFalseBot = !trueCloneFalseBot;
                    }
                }
                else
        ...
        public void FireLaser(int type, float speed = 6f, float recoilMult = 2f, float ai1 = 0, float ai2 = 0, int damage = 50)
        {
            ...
            if (type == ModContent.ProjectileType<CyverLaser>() || type == ModContent.ProjectileType<StretchLaser>())
            {
                ...
                float addition = State == AIState.Idle ? -1 : 0;
                for (int i = 0; i < 7 + (addition * 2); i++) //4 //2,2
                {
                    ...
                    p.fadeIn = (State == AIState.Idle ? 36 : 46) + Main.rand.NextFloat(-5, 10);
                    p.velocity *= 0.4f;
                }
        ...
        public void ShotDust(float velMult = 1f)
        {
            ...
            for (int i = 0; i < 9; i++) //4 //2,2
            {
                ...
                p.noLight = false;
                //p.velocity += NPC.velocity * (0.8f + Main.rand.NextFloat(-0.1f, -0.2f));
                p.fadeIn = (State == AIState.Idle ? 38 : 48) + Main.rand.NextFloat(-5, 10);
                p.velocity *= 0.4f;
            }
