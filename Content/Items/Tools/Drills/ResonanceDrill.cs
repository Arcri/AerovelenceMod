...
namespace AerovelenceMod.Content.Items.Tools.Drills
{
    public class ResonanceDrill : ModItem
    {
        ...
        {
            return player.ownedProjectileCounts[ModContent.ProjectileType<ResonanceDrillProj>()] == 0;
        }
    ...
    {
        public override string Texture => "AerovelenceMod/Content/Items/Tools/Drills/ResonanceDrillProj";
        ...
        public override void SetDefaults()
        {
            _blackGlowTexture = Mod.Assets.Request<Texture2D>("Content/Items/Tools/Drills/ResonanceDrillProjGlowBlack").Value;
            _colorGlowTexture = Mod.Assets.Request<Texture2D>("Content/Items/Tools/Drills/ResonanceDrillProjGlow").Value;
            _pulseGlowTexture = Mod.Assets.Request<Texture2D>("Content/Items/Tools/Drills/ResonanceDrillDrillOrange").Value;
            _drillTexture = Mod.Assets.Request<Texture2D>("Content/Items/Tools/Drills/ResonanceDrillDrill").Value;
        ...
        public override void AI()
        {
            ...
            if (Main.GameUpdateCount % 3 == 0)
            {
                ...
                Dust gd = Dust.NewDustPerfect(tipPosition, ModContent.DustType<GlowPixelCross>(), dustVel, 0, Color.SkyBlue, Main.rand.NextFloat(0.2f, 0.4f));
    ...
    public class VeinTile(int x, int y, int tileType)
    {
        public int X = x;
        public int Y = y;
    ...
    public class ResonanceDrillGlobalTile : GlobalTile
    {
        public static HashSet<int> AllowedTiles =
        [
        ...
        public override void KillTile(int x, int y, int oldType, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            ...
            if (player.HeldItem.type == ModContent.ItemType<ResonanceDrill>() && player.channel && player.itemAnimation > 0)
            {
                if (AllowedTiles.Contains(oldType))
                    ResonanceDrillShockwaveSystem.QueueShockwave(new Point(x, y), oldType);
            }
        }
    }

    public class ShockwaveEvent(Point origin, int tileType, int delayTicks)
    {
        public Point Origin = origin;
        public int TileType = tileType;
        public int DelayTicksRemaining = delayTicks;
    }

    public class ResonanceDrillShockwaveSystem : ModSystem
    {
        public static List<ShockwaveEvent> ShockwaveEvents = [];

        public static void QueueShockwave(Point origin, int tileType)
        {
            const int delayTicks = 15;
            ShockwaveEvents.Add(new ShockwaveEvent(origin, tileType, delayTicks));
        }

        public override void PostUpdateEverything()
        {
            for (int i = ShockwaveEvents.Count - 1; i >= 0; i--)
            {
                ShockwaveEvent ev = ShockwaveEvents[i];
                ev.DelayTicksRemaining--;
                if (ev.DelayTicksRemaining <= 0)
                {
                    TriggerShockwave(ev);
                    ShockwaveEvents.RemoveAt(i);
                }
            }
        }

        private static void TriggerShockwave(ShockwaveEvent ev)
        {
            int radius = 1;
            for (int i = -radius; i <= radius; i++)
            {
                ...
                {
                    int xx = ev.Origin.X + i;
                    int yy = ev.Origin.Y + j;
                    if (WorldGen.InWorld(xx, yy))
                    {
                        Tile tile = Main.tile[xx, yy];
                        if (tile != null && tile.HasTile && tile.TileType == ev.TileType)

                            WorldGen.KillTile(xx, yy, false, false, false);
                    }
                }
            }
        }
