...
namespace AerovelenceMod.Content.Items.Weapons.Misc.Magic.Ceroba
{
    public class CerobaStaffItem : ModItem
    {
        public override void SetDefaults()
        {
            ...
            Item.width = 62;
            Item.height = 62;
            Item.value = Item.sellPrice(0, 1, 29, 23);
        ...
        {
            if (player.ownedProjectileCounts[ModContent.ProjectileType<CerobaIdleHeldProj>()] < 1)
    ...
    public class CerobaIdleHeldProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.inventory[player.selectedItem].type != ModContent.ItemType<CerobaStaffItem>())
            {
                Projectile.Kill();
            }
            ...
            {
                Vector2 vel = (Projectile.rotation + MathHelper.PiOver2).ToRotationVector2() * 5f;
        ...
        public void FaceRightLogic(Player player)
        {
            ...
            Projectile.rotation = MathHelper.Lerp(goalRot * -30, (goalRot + ((yOffset + xOffset) * 0.5f)), Easings.easeOutCubic(progress));
        ...
        public void FaceLeftLogic(Player player)
        {
            ...
            Projectile.rotation = MathHelper.Lerp(goalRot * -5.85f, (goalRot + ((-yOffset + -xOffset) * 0.5f)) * -1, Easings.easeOutCubic(progress));
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 bottomScale = dir == 1 ?
                new Vector2(scalePercent - (ribbonBottomSinRot < 0 ? ribbonBottomSinRot : ribbonBottomSinRot * 0.5f), 1f)
                : new Vector2(scalePercent - (ribbonBottomSinRot > 0 ? -ribbonBottomSinRot * 0.75f : -ribbonBottomSinRot * 0.5f), 1f);

            Vector2 topScale = new Vector2(1f, scalePercent - (ribbonTopSinRot < 0 ? ribbonTopSinRot : ribbonTopSinRot * 0.5f));
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Main.EntitySpriteDraw(White, armPos + Main.rand.NextVector2Circular(1.5f, 1.5f) + new Vector2(21f, -21f).RotatedBy(underGlowAdjustedRot), null, Color.Gold with { A = 0 } * alpha * 0.2f, underGlowAdjustedRot, White.Size() / 2, Projectile.scale, SpriteEffects.None);
            }
    ...
    public class CerobaFireBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (timer > 10)
            {
                if (timer % 4 + Main.rand.Next(1, 2) == 0)
                {
                    /*
                    Vector2 vel = -Projectile.velocity.SafeNormalize(Vector2.UnitX).RotatedBy(Main.rand.NextFloat(-0.25f, 0.25f)) * Main.rand.NextFloat(3, Projectile.velocity.Length() * 0.5f);

                    Dust p = Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<LineSpark>(), vel,
                        newColor: Color.DeepPink, Scale: Main.rand.NextFloat(0.45f, 0.65f) * 0.35f);

                    p.customData = DustBehaviorUtil.AssignBehavior_LSBase(velFadePower: 0.93f, preShrinkPower: 0.99f, postShrinkPower: 0.98f, timeToStartShrink: 5 + Main.rand.Next(-5, 5), killEarlyTime: 10,
                        1f, 0.5f);
                    */

                    Vector2 randomStart = Main.rand.NextVector2Circular(3.5f, 3.5f) * 1f;
                    Dust dust = Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlowPixelCross>(), randomStart, newColor: Color.HotPink, Scale: Main.rand.NextFloat(0.35f, 0.5f));
                    dust.velocity += Projectile.velocity * -0.25f;

                    dust.customData = DustBehaviorUtil.AssignBehavior_GPCBase(
                        rotPower: 0.15f, preSlowPower: 0.95f, timeBeforeSlow: 6, postSlowPower: 0.92f, velToBeginShrink: 3.5f, fadePower: 0.85f, shouldFadeColor: false);
                }
            }
    ...
    public class CerobaWard : ModProjectile
    {
        ...
        public override void AI()
        {
            float timeProgress = Utils.GetLerpValue(0f, 1f, timer / 55f, true);
            float alphaTime = Utils.GetLerpValue(0f, 1f, timer / 40f, true);

            Projectile.rotation = MathHelper.Lerp(MathHelper.TwoPi * -1.25f,  0f, Easings.easeOutQuart(timeProgress));
            ...
            if (timer >= 55f)
            {
                ...
                float flashProgress = Utils.GetLerpValue(0f, 1f, (timer - 55f) / 30f, true);
