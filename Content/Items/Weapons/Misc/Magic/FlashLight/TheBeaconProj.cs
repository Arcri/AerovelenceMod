    ...
    public class TheBeaconProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.frameCounter >= 4)
            {
                ...
                {
                    SoundStyle stylecs = new SoundStyle("Terraria/Sounds/Item_109") with { Pitch = .82f, PitchVariance = .41f, Volume = 0.4f };
                    SoundEngine.PlaySound(stylecs, Projectile.Center);

                    int a = Projectile.NewProjectile(null, Projectile.Center + new Vector2(25, -10).RotatedBy(Projectile.rotation), (Projectile.rotation + 0.2f).ToRotationVector2(), ModContent.ProjectileType<BeaconShot>(), Projectile.damage / 3, 0, player.whoAmI);
                    int b = Projectile.NewProjectile(null, Projectile.Center + new Vector2(25, 10).RotatedBy(Projectile.rotation), (Projectile.rotation - 0.2f).ToRotationVector2(), ModContent.ProjectileType<BeaconShot>(), Projectile.damage / 3, 0, player.whoAmI);
                    int c = Projectile.NewProjectile(null, Projectile.Center + new Vector2(25, 0).RotatedBy(Projectile.rotation), Projectile.rotation.ToRotationVector2(), ModContent.ProjectileType<BeaconShot>(), Projectile.damage / 3, 0, player.whoAmI);
                    //Projectile.frameCounter = -24;
            ...
            if (FindNearestNPCMouse(250f, true, false, true, out int index))
            {
                    ...
                    if (timer % 2 == 0)
                    {
                        ...
                        if (Main.rand.NextBool())
                        {
                            for (int i = 0; i < 2; i++)
                            {
                                ...
                                Dust gd = GlowDustHelper.DrawGlowDustPerfect(npc.Center, ModContent.DustType<LineGlow>(), randomStart * Main.rand.NextFloat(0.65f, 1.35f), Color.Firebrick, 0.15f, 0.2f, 0f, dustShader2);
                        ...
                        else
                        {
                            for (int i = 0; i < 2; i++)
                            {
                                ...
                                Dust gd = GlowDustHelper.DrawGlowDustPerfect(npc.Center, ModContent.DustType<GlowCircleFlare>(), randomStart * Main.rand.NextFloat(0.65f, 1.35f), Color.Firebrick, 0.3f, 0.1f, 0f, dustShader2);
                                gd.fadeIn = 2;
                            }
                        }
                    }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, sourceRectangle, lightColor, direction.ToRotation() + MathHelper.PiOver2, origin332, Projectile.scale, spriteEffects, 0.0f);
            Main.spriteBatch.Draw(glowMask, Projectile.Center - Main.screenPosition, sourceRectangle, Color.White * 0.3f, direction.ToRotation() + MathHelper.PiOver2, origin332, Projectile.scale, spriteEffects, 0.0f);
            //Main.spriteBatch.Draw(glowMask, Projectile.Center - Main.screenPosition, sourceRectangle, (Color.White * 0.1f) * (laserWidth * 0.1f), direction.ToRotation() + MathHelper.PiOver2, origin332, Projectile.scale + 0.1f, spriteEffects, 0.0f);
            ...
            myEffect.Parameters["uColor"].SetValue(Color.Firebrick.ToVector3() * 0.25f);
            ...
            myEffect.Parameters["uTime"].SetValue(timer * -0.02f); //0.006
            ...
            float height = (laserWidth * 1f); //25

            int width = (int)((Projectile.Center + (direction.SafeNormalize(Vector2.UnitX) * 40)) - endPoint).Length();

            var pos = Projectile.Center + (direction.SafeNormalize(Vector2.UnitX) * 40) - Main.screenPosition;
            var target = new Rectangle((int)pos.X, (int)pos.Y, width, (int)(height * 1.1f));
            ...
            var target2 = new Rectangle((int)pos.X, (int)pos.Y, width, (int)(height * 1.7f));
            var target3 = new Rectangle((int)pos.X, (int)pos.Y, width, (int)(height * 1.3f));

            Main.spriteBatch.Draw(LaserTexture, target2, null, Color.Orange * 0.5f, LaserRotation, origin2, 0, 0);
            Main.spriteBatch.Draw(LaserTexture, target3, null, Color.Orange * 0.25f, LaserRotation, origin2, 0, 0);

            //Flares
            Color flareCol = new Color(181, 64, 13);
            ...
            myEffect2.Parameters["uColor"].SetValue(flareCol.ToVector3() * 2.5f);
            ...
            Main.spriteBatch.Draw(flare12, Projectile.Center + (direction.SafeNormalize(Vector2.UnitX) * 40) - Main.screenPosition, flare12.Frame(1,1,0,0), Color.Goldenrod, timer * 0.05f, flare12.Size() / 2, 0.2f * laserWidth * 0.02f, spriteEffects, 0.0f);
            Main.spriteBatch.Draw(flare1, endPoint - Main.screenPosition, flare1.Frame(1, 1, 0, 0), Color.Goldenrod, timer * 0.07f, flare1.Size() / 2, 0.01f * laserWidth, spriteEffects, 0.0f);
