...
{
    public class WoodSlug : WoodSlugSlugItem
    {
        protected override WoodSlugSlugMaterial Material => WoodSlugSlugMaterial.Wood;
        protected override string EnglishTooltip => "A light sling slug with a long, shallow arc\nWell-timed throws deal 1.5x Skill Strike damage";
        public override void SetStaticDefaults()
        {
            this.ModifyLocalization("Wood Slug", EnglishTooltip)
                .AddName(Language.Spanish, "Proyectil de Madera")
                .AddTooltip(Language.Spanish, "Un proyectil ligero para honda con una trayectoria larga y poco pronunciada\nLos lanzamientos bien sincronizados infligen 1,5 veces el daño con un Golpe de Habilidad");
            base.SetStaticDefaults();
        }
        public override void AddRecipes() => CreateRecipe(50).AddIngredient(ItemID.Wood, 5).AddTile(TileID.WorkBenches).Register();
    }
    public class WoodSlugShot : WoodSlugSlugProjectile
    {
        protected override WoodSlugSlugMaterial Material => WoodSlugSlugMaterial.Wood;
    }

    public enum WoodSlugSlugMaterial { Stone, Wood, Crystal }

    public abstract class WoodSlugSlugItem : TranslatableModItem
    {
        protected abstract WoodSlugSlugMaterial Material { get; }
        protected abstract string EnglishTooltip { get; }
        public override string Texture => "AerovelenceMod/Content/Items/Ammo/WoodSlug";
        public override void SetDefaults()
        {
            ...
            Item.damage = Material == WoodSlugSlugMaterial.Stone ? 5 : Material == WoodSlugSlugMaterial.Wood ? 2 : 4;
            Item.DamageType = DamageClass.Ranged;
            Item.knockBack = Material == WoodSlugSlugMaterial.Stone ? 4f : Material == WoodSlugSlugMaterial.Wood ? 2f : 3f;
            Item.ammo = ModContent.ItemType<StoneSlug>();
            Item.shoot = Material == WoodSlugSlugMaterial.Stone ? ModContent.ProjectileType<StoneSlugShot>() :
                Material == WoodSlugSlugMaterial.Wood ? ModContent.ProjectileType<WoodSlugShot>() : ModContent.ProjectileType<CrystalStoneSlugShot>();
            Item.shootSpeed = 1f;
    ...
    public abstract class WoodSlugSlugProjectile : ModProjectile
    {
        protected abstract WoodSlugSlugMaterial Material { get; }
        ...
        {
            if (SkillStrike)
                SkillStrikeUtil.setSkillStrike(Projectile, Material == WoodSlugSlugMaterial.Stone ? 2.25f : Material == WoodSlugSlugMaterial.Wood ? 1.5f : 2f, 1, 0.35f, 0.65f);
        }
        public override void AI()
        {
            ...
            if (Projectile.ai[1]++ == 0f)
            {
                Projectile.velocity *= Material == WoodSlugSlugMaterial.Stone ? 0.9f : Material == WoodSlugSlugMaterial.Wood ? 1.25f : 1f;
                SoundEngine.PlaySound(SoundID.Item1 with { Volume = 0.55f, Pitch = SkillStrike ? 0.55f : 0.1f }, Projectile.Center);
                if (SkillStrike)
                {
                    WoodSlugVFX.Burst(Projectile.Center, 10, 4f);
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<WoodSlugImpact>(),
                            0, 0f, Projectile.owner, 0.5f, Projectile.velocity.ToRotation());
                    }
                }
            }
            float gravity = Material == WoodSlugSlugMaterial.Stone ? 0.36f : Material == WoodSlugSlugMaterial.Wood ? 0.1f : 0.23f;
            Projectile.velocity.Y = Math.Min(18f, Projectile.velocity.Y + gravity);
            Projectile.velocity.X *= Material == WoodSlugSlugMaterial.Wood ? 0.999f : 0.997f;
            Projectile.rotation += Projectile.velocity.X * 0.06f;
            if ((SkillStrike || Material == WoodSlugSlugMaterial.Crystal) && Projectile.ai[1] % 3f == 0f)
                WoodSlugVFX.Spark(Projectile.Center, -Projectile.velocity * 0.06f, 0.17f * Opacity, SkillStrike ? Color.White : WoodSlugSlugArt.Tint(Material));
            if (Material == WoodSlugSlugMaterial.Stone && Projectile.ai[1] < 15f && Projectile.ai[1] % 4f == 0f)
                WoodSlugVFX.Smoke(Projectile.Center, -Projectile.velocity * 0.06f, 18f, new Color(125, 140, 160));
        }
        public override void OnKill(int timeLeft)
        {
            if (timeLeft <= 0 || Main.dedServ)
                return;
            Color tint = WoodSlugSlugArt.Tint(Material);
            WoodSlugVFX.Smoke(Projectile.Center, -Projectile.velocity * 0.05f, SkillStrike ? 55f : 32f, tint * 0.65f);
            if (Material == WoodSlugSlugMaterial.Wood)
            {
                for (int i = 0; i < 7; i++)
                    Dust.NewDustPerfect(Projectile.Center, DustID.WoodFurniture, Main.rand.NextVector2Circular(3.5f, 3.5f), Scale: 0.9f);
            }
            else
                WoodSlugVFX.Rubble(Projectile.Center, SkillStrike ? 7 : 4, SkillStrike ? 4f : 2.5f);
            if (SkillStrike || Material == WoodSlugSlugMaterial.Crystal)
                WoodSlugVFX.Burst(Projectile.Center, SkillStrike ? 12 : 7, SkillStrike ? 4f : 2.5f);
            SoundEngine.PlaySound(Material == WoodSlugSlugMaterial.Crystal ? SoundID.Shatter with { Volume = 0.3f, Pitch = 0.15f } :
                SoundID.Dig with { Volume = SkillStrike ? 0.6f : 0.4f, Pitch = Material == WoodSlugSlugMaterial.Stone ? -0.25f : 0.15f }, Projectile.Center);
            if (SkillStrike && Projectile.owner == Main.myPlayer)
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<WoodSlugImpact>(), 0, 0f,
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = Projectile.oldPos.Length - 1; i > 0; i--)
            {
                ...
                WoodSlugSlugArt.Draw(Main.spriteBatch, Projectile.oldPos[i] + Projectile.Size * 0.5f - Main.screenPosition, Material,
                    Projectile.oldRot[i], 12f * (0.7f + fade * 0.3f), SkillStrike ? WoodSlugVFX.Additive(tint, fade * 0.4f) : lightColor * (fade * 0.15f), 0f, fade);
            }
            if (SkillStrike)
                WoodSlugVFX.Glow(Projectile.Center, new Vector2(36f), tint, Opacity * 0.45f);
            WoodSlugSlugArt.Draw(Main.spriteBatch, Projectile.Center - Main.screenPosition, Material, Projectile.rotation, 14f,
                lightColor, SkillStrike ? 0.65f : 0f, Opacity);
            return false;
        }
    }

    internal static class WoodSlugSlugArt
    {
        internal static Color Tint(WoodSlugSlugMaterial material) => material switch
        {
            WoodSlugSlugMaterial.Wood => new Color(220, 177, 115),
            WoodSlugSlugMaterial.Crystal => WoodSlugVFX.Aqua,
            _ => new Color(155, 180, 218)
        };
        internal static void Draw(SpriteBatch spriteBatch, Vector2 center, WoodSlugSlugMaterial material, float angle, float size, Color light, float charge, float opacity = 1f)
        {
            string asset = "AerovelenceMod/Content/Items/Ammo/WoodSlug";
            ...
            if (charge > 0f)
                spriteBatch.Draw(texture, center, null, WoodSlugVFX.Additive(Color.White, charge * opacity * 0.6f), angle, texture.Size() * 0.5f, scale, SpriteEffects.None, 0f);
        }
    }

    public class WoodSlugImpact : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            WoodSlugVFX.Glow(Projectile.Center, new Vector2(size * 1.5f), WoodSlugVFX.Violet, fade * 0.5f);
            WoodSlugVFX.Ring(Projectile.Center, new Vector2(size, size * 0.7f), WoodSlugVFX.Aqua, fade * 0.65f, Projectile.ai[1]);
            WoodSlugVFX.Sprite(Texture, Projectile.Center, new Vector2(size * 0.6f), WoodSlugVFX.Additive(WoodSlugVFX.Aqua, fade * 0.4f), Projectile.identity * 2.3f);
            WoodSlugVFX.Flare(Projectile.Center, size * 1.3f, Math.Max(0f, 1f - progress * 3f), Projectile.ai[1]);
            return false;
        }
    }

    public class WoodSlugDebris : ModDust
    {
        ...
        public override bool PreDraw(Dust dust)
        {
            ...
            Main.EntitySpriteDraw(glow, position, dust.frame, WoodSlugVFX.Additive(WoodSlugVFX.Aqua, opacity * 0.7f), dust.rotation, dust.frame.Size() * 0.5f, dust.scale, SpriteEffects.None);
            return false;
        }
    }

    internal static class WoodSlugVFX
    {
        ...
        internal static void Rubble(Vector2 center, int count, float speed = 4f)
        {
            if (Main.dedServ)
                return;
            for (int i = 0; i < count; i++)
                Dust.NewDustPerfect(center, ModContent.DustType<WoodSlugDebris>(), new Vector2(Main.rand.NextFloat(-speed, speed), Main.rand.NextFloat(-speed * 1.3f, -1f)), Scale: Main.rand.NextFloat(0.35f, 0.75f));
        }
    }
}
